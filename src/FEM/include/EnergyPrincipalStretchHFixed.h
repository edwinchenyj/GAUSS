//
//  EnergyPrincipalStretchHFixed.h
//  Gauss
//
//  Created by Yu Ju Edwin Chen on 2018-12-13.
//

#ifndef EnergyPrincipalStretchHFixed_h
#define EnergyPrincipalStretchHFixed_h
...
class EnergyPrincipalStretchHFixed : public virtual ShapeFunction {
public:
    template<typename QDOFList, typename QDotDOFList>
    EnergyPrincipalStretchHFixed(Eigen::MatrixXx<DataType> &V, Eigen::MatrixXi &F, QDOFList &qDOFList, QDotDOFList &qDotDOFList) : ShapeFunction(V, F, qDOFList, qDotDOFList) {
...
//
        //check multiplicity, small random permutation if eigenvalues not unique
        if(std::fabs(m_S[0] - m_S[1]) < 1e-4 || std::fabs(m_S[1] - m_S[2]) < 1e-4 || std::fabs(m_S[0] - m_S[2]) < 1e-4) {
            F += Eigen::Matrix33x<float>::Random()*1e-4;
            igl::svd3x3(F,m_U,m_S,m_V);
        }
...
#endif /* EnergyPrincipalStretchHFixed_h */
