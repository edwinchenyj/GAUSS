...
//  Gauss
//
//  Created by Edwin Chen on 2018-11-11.
//
//
//#define EDWIN_DEBUG

#ifndef EigenFitLinear_h
#define EigenFitLinear_h
...
public:
    ...
    EigenFitLinear(Eigen::MatrixXx<double> &Vc, Eigen::MatrixXi &Fc,Eigen::MatrixXx<double> &Vf, Eigen::MatrixXi &Ff, int dynamic_switch, double youngs, double poisson, int constraintDir, double constraintTol, unsigned int cswitch, unsigned int hausdorff_dist, unsigned int numModes, std::string cmeshname, std::string fmeshname, Eigen::VectorXx<double> ratio_manual, int compute_frequency ) : PhysicalSystemImpl(Vc,Fc)
    {
        step_number = 0;
    ...
    ~EigenFitLinear() {delete fine_pos0;
    }
        ...
        if((!ratio_calculated))
        {
            ...
            else if (ratio_recalculation_switch == 2)
            {
                if(step_number%m_compute_frequency == 0) // update once every few frames
                {
                    ...
                    restartEigenvalues(m_Vc_current,m_Fc,m_coarseP,coarse_eval);
                    
                    
                    Eigen::VectorXd fine_eval;
                    restartEigenvalues(m_Vf_current,m_Ff,m_fineP,fine_eval);
