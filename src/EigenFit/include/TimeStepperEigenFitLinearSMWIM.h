//
//  TimeStepperEigenFitLinearSMWIM.h
//  Gauss
//
//  Created by Edwin Chen on 2018-11-15.
//
//

#ifndef TimeStepperEigenFitLinearSMWIM_h
#define TimeStepperEigenFitLinearSMWIM_h
...
namespace Gauss {
    ...
    class TimeStepperImplEigenFitLinearSMWIMImpl
    {
    public:
        
        template<typename Matrix>
        TimeStepperImplEigenFitLinearSMWIMImpl(Matrix &P, unsigned int numModes) {
        ...
        TimeStepperImplEigenFitLinearSMWIMImpl(const TimeStepperImplEigenFitLinearSMWIMImpl &toCopy) {
            
        }
        
        ~TimeStepperImplEigenFitLinearSMWIMImpl() { }
...
template<typename DataType, typename MatrixAssembler, typename VectorAssembler>
template<typename World>
void TimeStepperImplEigenFitLinearSMWIMImpl<DataType, MatrixAssembler, VectorAssembler>::step(World &world, double dt, double t) {
    
    // TODO: should not be here... set the rayleigh damping parameter
    a = static_cast<EigenFitLinear*>(std::get<0>(world.getSystemList().getStorage())[0])->a;
    b = static_cast<EigenFitLinear*>(std::get<0>(world.getSystemList().getStorage())[0])->b;
    ...
    do {
        ...
        {
            cout<<"Ignoring change in stiffness matrix from EigenFitLinear"<<endl;
        }
        else
        {
            //
        ...
        //
        //                (*forceVector) = m_P*(*forceVector);
        //                static_cast<EigenFitLinear*>(std::get<0>(world.getSystemList().getStorage())[0])->calculateEigenFitData(q,massMatrix,stiffnessMatrix,m_coarseUs,Y,Z);
        //
        //                //    Correct Forces
        //                (*forceVector) = (*forceVector) + Y*m_coarseUs.first.transpose()*(*forceVector);
        //
...
template<typename DataType, typename MatrixAssembler, typename VectorAssembler>
using TimeStepperEigenFitLinearSMWIM = TimeStepper<DataType, TimeStepperImplEigenFitLinearSMWIMImpl<DataType, MatrixAssembler, VectorAssembler> >;




#endif /* TimeStepperEigenFitLinearSMWIM_h */
